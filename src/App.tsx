...
type ServerMessage = { type: string; rooms?: Room[]; room?: Room; board?: (string | null)[]; turn?: string; role?: 'red' | 'black' | ''; players?: string[]; messages?: ChatMessage[]; username?: string; message?: string }
...
function App() {
  ...
  const [role, setRole] = useState<'red' | 'black' | ''>('')
  ...
  useEffect(() => {
    ...
    connection.onmessage = (event) => {
      ...
      if (message.type === 'room_state' && message.room) { setActiveRoom(message.room); setGame(message.room.game); setBoard(message.board || []); setTurn(message.turn || 'red'); setRole(message.role || ''); setPlayers(message.players || []); setMessages(message.messages || []); setSelected(null) }
